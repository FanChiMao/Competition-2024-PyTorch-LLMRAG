...
def explain_proper_noun(text):
    explain_dict = {
        "realtek": "瑞昱",
        "瑞昱": "realtek",

    }
    for noun in explain_dict.keys():
        if re.search(noun, text, re.IGNORECASE):
            text = re.sub(noun, noun+explain_dict[noun], text, flags=re.IGNORECASE)
