...
class BaseReranker:
    ...
    def _rerank_with_reranker(self, query: str, top_docs: list[str]):
        ...
        for doc in top_docs:
            chunk_texts = self._chunk_document_str(text=doc)
    ...
    def _chunk_document_str(self, text: str):
        """
        ...
        if stride <= 0:
            raise ValueError(f"Overlap size must be smaller than chunk size, get {self.overlap_size}.")
